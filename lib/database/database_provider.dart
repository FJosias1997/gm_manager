...
class OfflineDatabaseProvider {
  ...
  Future<int> saveRelatorioAnual({required Map<String, dynamic> data}) async {
    final db = await database;
  ...
  Future<int> saveGM({required Map<String, dynamic> data}) async {
    final db = await database;
  ...
  Future<int> deleteRelatorioAnual(String id) async {
    ...
    try {
      return await db.rawDelete('DELETE FROM relatorio_anual WHERE id = ?', [
        id,
      ]);
    } catch (e) {
      debugPrint(e.toString());
